...
A full-stack task, project, and budget tracker built with Laravel. Plan projects, break them into tasks and subtasks, and keep monthly budgets in check — all in one workspace, with a role-gated admin panel for cross-user reporting.

[![PHP](https://img.shields.io/badge/PHP-8.2%2B-777BB4?logo=php&logoColor=white)](https://www.php.net/)
[![Laravel](https://img.shields.io/badge/Laravel-11-FF2D20?logo=laravel&logoColor=white)](https://laravel.com/)
[![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-3-06B6D4?logo=tailwindcss&logoColor=white)](https://tailwindcss.com/)
[![Tests](https://img.shields.io/badge/tests-Pest-8B5CF6)](https://pestphp.com/)
[![License: MIT](https://img.shields.io/badge/license-MIT-informational)](LICENSE)

**[Live demo](https://budget-management-system-production.up.railway.app)** — log in with the [demo account](#demo-account) below, no signup required.
...
| | |
|---|---|
| **Auth** | Registration/login, OTP-based password reset delivered by email |
| **Projects** | Start/end dates, status tracking, and a visual progress bar |
...
| **Admin panel** | Role-gated: manage users, view all tasks, spending trends, team productivity |
| **Calendar view** | All projects plotted by date range |

## Tech stack

- **Backend:** PHP 8.2+, Laravel 11
- **Frontend:** Vite 6, Tailwind CSS 3
- **Database:** SQLite (zero-config local dev) or MySQL/PostgreSQL in production
- **Mail:** [Resend](https://resend.com) for transactional email (OTP delivery)
- **Testing:** Pest
...
Idempotent — running the seeder again won't duplicate its data.
...
The repo includes a `Procfile` for Heroku-style / Nixpacks-style platforms (Railway, Heroku, etc.). It runs migrations and the admin/demo seeders every time the container starts, then serves the app — most of these platforms (Railway included) don't reliably run a Heroku-style `release` step, so it's folded into the `web` process instead. Requirements:
...
| `ADMIN_EMAIL` / `ADMIN_PASSWORD` | creates the initial admin account when the seeder runs |
...
php artisan db:seed --force        # creates the admin + demo accounts; safe to re-run
...
4. **Deploy** — Railway builds automatically, detects `composer.json`/`artisan`, and picks up the `Procfile` for the start command. Migrations and seeding run automatically on every container start — no manual step needed.
